...
// Forzamos el nombre a: NombreAlumno NombreCurso
            if ($customcert && isset($user)) {
                $coursefullname = $DB->get_field('course', 'fullname', ['id' => $customcert->course]);
                $userfullname = format_text(fullname($user), FORMAT_PLAIN);
                $coursename = format_string($coursefullname);
                
                // Sobrescribimos el filename
                $filename = $userfullname . ' ' . $coursename;
            }
